...
export function useGetCurrencysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*currency.CurrencyEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: currency.CurrencyEntity[] = query$.data?.data?.items;
  const items = [];
