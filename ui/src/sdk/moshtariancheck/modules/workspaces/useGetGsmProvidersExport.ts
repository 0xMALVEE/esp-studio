...
export function useGetGsmProvidersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getGsmProvidersExport();
  ...
  const query$ = useQuery(
    ["*workspaces.GsmProviderEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: workspaces.GsmProviderEntity[] = query$.data?.data?.items;
  const items = [];
