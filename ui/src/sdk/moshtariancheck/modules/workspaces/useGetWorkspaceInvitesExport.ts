...
export function useGetWorkspaceInvitesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getWorkspaceInvitesExport();
  ...
  const query$ = useQuery(
    ["*workspaces.WorkspaceInviteEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: workspaces.WorkspaceInviteEntity[] = query$.data?.data?.items;
  const items = [];
