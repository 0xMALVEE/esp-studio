...
export function useGetAccountCollectionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAccountCollectionsExport();
  ...
  const query$ = useQuery(
    ["*accounting.AccountCollectionEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AccountCollectionEntity[] = query$.data?.data?.items;
  const items = [];
