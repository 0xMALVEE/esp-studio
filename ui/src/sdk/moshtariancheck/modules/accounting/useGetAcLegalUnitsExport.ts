...
export function useGetAcLegalUnitsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcLegalUnitsExport();
  ...
  const query$ = useQuery(
    ["*accounting.AcLegalUnitEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AcLegalUnitEntity[] = query$.data?.data?.items;
  const items = [];
