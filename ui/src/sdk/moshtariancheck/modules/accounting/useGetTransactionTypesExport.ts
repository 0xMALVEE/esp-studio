...
export function useGetTransactionTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getTransactionTypesExport();
  ...
  const query$ = useQuery(
    ["*accounting.TransactionTypeEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.TransactionTypeEntity[] = query$.data?.data?.items;
  const items = [];
