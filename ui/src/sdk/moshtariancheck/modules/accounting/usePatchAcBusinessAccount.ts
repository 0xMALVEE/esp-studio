...
export function usePatchAcBusinessAccount({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchAcBusinessAccount(entity);

  const mutation = useMutation<
    IResponse<accounting.AcBusinessAccountEntity>,
    IResponse<accounting.AcBusinessAccountEntity>,
    Partial<accounting.AcBusinessAccountEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<accounting.AcBusinessAccountEntity>,
    formikProps?: FormikHelpers<Partial<accounting.AcBusinessAccountEntity>>
