...
export function useGetAcCheckStatussExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcCheckStatussExport();
  ...
  const query$ = useQuery(
    ["*accounting.AcCheckStatusEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AcCheckStatusEntity[] = query$.data?.data?.items;
  const items = [];
