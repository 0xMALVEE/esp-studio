...
export function useGetAcBankBranchByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getAcBankBranchByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getAcBankBranchByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
