...
export function useGetAcCheckStatusByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getAcCheckStatusByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getAcCheckStatusByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
