...
export function usePatchGeoProvince({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchGeoProvince(entity);

  const mutation = useMutation<
    IResponse<geo.GeoProvinceEntity>,
    IResponse<geo.GeoProvinceEntity>,
    Partial<geo.GeoProvinceEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<geo.GeoProvinceEntity>,
    formikProps?: FormikHelpers<Partial<geo.GeoProvinceEntity>>
