...
export function usePatchGeoLocations({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchGeoLocations(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<geo.GeoLocationEntity>>,
    IResponse<core.BulkRecordRequest<geo.GeoLocationEntity>>,
    Partial<core.BulkRecordRequest<geo.GeoLocationEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<geo.GeoLocationEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<geo.GeoLocationEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<geo.GeoLocationEntity>>,
