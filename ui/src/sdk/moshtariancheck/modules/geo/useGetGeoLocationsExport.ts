...
export function useGetGeoLocationsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getGeoLocationsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*geo.GeoLocationEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: geo.GeoLocationEntity[] = query$.data?.data?.items;
  const items = [];
