...
export function useGetLocationDatasExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getLocationDatasExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*geo.LocationDataEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: geo.LocationDataEntity[] = query$.data?.data?.items;
  const items = [];
