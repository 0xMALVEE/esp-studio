...
export function useGetGeoLocationByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getGeoLocationByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getGeoLocationByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
