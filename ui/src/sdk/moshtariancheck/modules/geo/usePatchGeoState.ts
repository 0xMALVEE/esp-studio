...
export function usePatchGeoState({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchGeoState(entity);

  const mutation = useMutation<
    IResponse<geo.GeoStateEntity>,
    IResponse<geo.GeoStateEntity>,
    Partial<geo.GeoStateEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<geo.GeoStateEntity>,
    formikProps?: FormikHelpers<Partial<geo.GeoStateEntity>>
