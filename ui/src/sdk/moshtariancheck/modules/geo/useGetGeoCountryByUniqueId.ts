...
export function useGetGeoCountryByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getGeoCountryByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getGeoCountryByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
