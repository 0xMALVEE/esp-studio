...
export function useGetGeoLocationTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getGeoLocationTypesExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*geo.GeoLocationTypeEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: geo.GeoLocationTypeEntity[] = query$.data?.data?.items;
  const items = [];
