...
export function useGetCustomerTypeByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getCustomerTypeByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getCustomerTypeByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
