...
export function useGetBusinessCustomersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getBusinessCustomersExport();
  ...
  const query$ = useQuery(
    ["*loyalty.BusinessCustomerEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: loyalty.BusinessCustomerEntity[] = query$.data?.data?.items;
  const items = [];
