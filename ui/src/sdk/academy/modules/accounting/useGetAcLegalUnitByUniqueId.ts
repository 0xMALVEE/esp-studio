...
export function useGetAcLegalUnitByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getAcLegalUnitByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getAcLegalUnitByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
