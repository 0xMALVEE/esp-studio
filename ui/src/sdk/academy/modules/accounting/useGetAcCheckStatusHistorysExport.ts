...
export function useGetAcCheckStatusHistorysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcCheckStatusHistorysExport();
  ...
  const query$ = useQuery(
    ["*accounting.AcCheckStatusHistoryEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AcCheckStatusHistoryEntity[] = query$.data?.data?.items;
  const items = [];
