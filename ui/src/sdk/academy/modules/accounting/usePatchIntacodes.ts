...
export function usePatchIntacodes({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchIntacodes(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<accounting.IntacodeEntity>>,
    IResponse<core.BulkRecordRequest<accounting.IntacodeEntity>>,
    Partial<core.BulkRecordRequest<accounting.IntacodeEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<accounting.IntacodeEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<accounting.IntacodeEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<accounting.IntacodeEntity>>,
