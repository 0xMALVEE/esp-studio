...
export function useGetAcBankBranchsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcBankBranchsExport();
  ...
  const query$ = useQuery(
    ["*accounting.AcBankBranchEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AcBankBranchEntity[] = query$.data?.data?.items;
  const items = [];
