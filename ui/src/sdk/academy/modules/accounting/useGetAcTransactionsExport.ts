...
export function useGetAcTransactionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcTransactionsExport();
  ...
  const query$ = useQuery(
    ["*accounting.AcTransactionEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AcTransactionEntity[] = query$.data?.data?.items;
  const items = [];
