...
export function useGetAcChecksExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*accounting.AcCheckEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: accounting.AcCheckEntity[] = query$.data?.data?.items;
  const items = [];
