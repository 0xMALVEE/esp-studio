...
export function useGetAcBusinessAccounts({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcBusinessAccounts();
  ...
  const query$ = useQuery(
    ["*accounting.AcBusinessAccountEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: accounting.AcBusinessAccountEntity[] = query$.data?.data?.items;
  const items = [];
