...
export function useGetAcTransactionByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getAcTransactionByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getAcTransactionByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
