...
export function useGetLoyaltyCustomersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getLoyaltyCustomersExport();
  ...
  const query$ = useQuery(
    ["*loyalty.LoyaltyCustomerEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: loyalty.LoyaltyCustomerEntity[] = query$.data?.data?.items;
  const items = [];
