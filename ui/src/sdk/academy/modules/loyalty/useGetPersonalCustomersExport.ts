...
export function useGetPersonalCustomersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getPersonalCustomersExport();
  ...
  const query$ = useQuery(
    ["*loyalty.PersonalCustomerEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: loyalty.PersonalCustomerEntity[] = query$.data?.data?.items;
  const items = [];
