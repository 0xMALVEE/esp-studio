...
export function useGetCustomerTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getCustomerTypesExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*loyalty.CustomerTypeEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: loyalty.CustomerTypeEntity[] = query$.data?.data?.items;
  const items = [];
