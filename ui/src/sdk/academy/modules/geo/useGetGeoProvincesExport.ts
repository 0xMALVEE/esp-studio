...
export function useGetGeoProvincesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getGeoProvincesExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*geo.GeoProvinceEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: geo.GeoProvinceEntity[] = query$.data?.data?.items;
  const items = [];
