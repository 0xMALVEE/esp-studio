...
export function useGetGeoStatesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*geo.GeoStateEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: geo.GeoStateEntity[] = query$.data?.data?.items;
  const items = [];
