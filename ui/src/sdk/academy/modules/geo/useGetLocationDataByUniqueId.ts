...
export function useGetLocationDataByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getLocationDataByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getLocationDataByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
