...
export function useGetShopProductCategorysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getShopProductCategorysExport();
  ...
  const query$ = useQuery(
    ["*shop.ShopProductCategoryEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: shop.ShopProductCategoryEntity[] = query$.data?.data?.items;
  const items = [];
