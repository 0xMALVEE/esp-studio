...
export function useGetLicensableProductsExport({
  ...
  const fn = () => Q().getLicensableProductsExport();
  ...
  const query$ = useQuery(
    ["*licenses.LicensableProductEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: licenses.LicensableProductEntity[] = query$.data?.data?.items;
  const items = [];
