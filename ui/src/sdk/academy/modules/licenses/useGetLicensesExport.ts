...
export function useGetLicensesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*licenses.LicenseEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: licenses.LicenseEntity[] = query$.data?.data?.items;
  const items = [];
