...
export function useGetMeasureUnitsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getMeasureUnitsExport();
  ...
  const query$ = useQuery(
    ["*measurement.MeasureUnitEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: measurement.MeasureUnitEntity[] = query$.data?.data?.items;
  const items = [];
