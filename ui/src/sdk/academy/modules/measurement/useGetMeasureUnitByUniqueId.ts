...
export function useGetMeasureUnitByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getMeasureUnitByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getMeasureUnitByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
