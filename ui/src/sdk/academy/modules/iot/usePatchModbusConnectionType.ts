...
export function usePatchModbusConnectionType({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchModbusConnectionType(entity);

  const mutation = useMutation<
    IResponse<iot.ModbusConnectionTypeEntity>,
    IResponse<iot.ModbusConnectionTypeEntity>,
    Partial<iot.ModbusConnectionTypeEntity>
  >(fn);
  ...
  const fnUpdater = (
    ...
    //     if (
    //       item.data !== undefined &&
    //       ModbusConnectionTypeActions.isModbusConnectionTypeEntityEqual(t, item.data)
    //     ) {
    //       return item.data;
    //     }
  ...
  const submit = (
    values: Partial<iot.ModbusConnectionTypeEntity>,
    formikProps?: FormikHelpers<Partial<iot.ModbusConnectionTypeEntity>>
