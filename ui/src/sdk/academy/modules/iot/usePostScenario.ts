...
export function usePostScenario({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().postScenario(entity);

  const mutation = useMutation<
    IResponse<iot.ScenarioEntity>,
    IResponse<iot.ScenarioEntity>,
    Partial<iot.ScenarioEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<iot.ScenarioEntity>,
    formikProps?: FormikHelpers<Partial<iot.ScenarioEntity>>
