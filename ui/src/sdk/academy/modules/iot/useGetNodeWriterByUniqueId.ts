...
export function useGetNodeWriterByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getNodeWriterByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getNodeWriterByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
