...
export function usePatchGpioMode({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchGpioMode(entity);

  const mutation = useMutation<
    IResponse<iot.GpioModeEntity>,
    IResponse<iot.GpioModeEntity>,
    Partial<iot.GpioModeEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<iot.GpioModeEntity>,
    formikProps?: FormikHelpers<Partial<iot.GpioModeEntity>>
