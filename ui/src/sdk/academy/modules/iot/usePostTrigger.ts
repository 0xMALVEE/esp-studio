...
export function usePostTrigger({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().postTrigger(entity);

  const mutation = useMutation<
    IResponse<iot.TriggerEntity>,
    IResponse<iot.TriggerEntity>,
    Partial<iot.TriggerEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<iot.TriggerEntity>,
    formikProps?: FormikHelpers<Partial<iot.TriggerEntity>>
