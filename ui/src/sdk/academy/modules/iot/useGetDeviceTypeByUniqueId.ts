...
export function useGetDeviceTypeByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getDeviceTypeByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getDeviceTypeByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
