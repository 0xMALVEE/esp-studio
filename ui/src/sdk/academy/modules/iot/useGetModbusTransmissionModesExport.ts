...
export function useGetModbusTransmissionModesExport({
  ...
  const fn = () => Q().getModbusTransmissionModesExport();
  ...
  const query$ = useQuery(
    ["*iot.ModbusTransmissionModeEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: iot.ModbusTransmissionModeEntity[] = query$.data?.data?.items;
  const items = [];
