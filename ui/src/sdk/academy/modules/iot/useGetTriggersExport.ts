...
export function useGetTriggersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*iot.TriggerEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: iot.TriggerEntity[] = query$.data?.data?.items;
  const items = [];
