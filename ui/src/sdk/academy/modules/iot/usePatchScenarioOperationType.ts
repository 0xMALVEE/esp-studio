...
export function usePatchScenarioOperationType({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchScenarioOperationType(entity);

  const mutation = useMutation<
    IResponse<iot.ScenarioOperationTypeEntity>,
    IResponse<iot.ScenarioOperationTypeEntity>,
    Partial<iot.ScenarioOperationTypeEntity>
  >(fn);
  ...
  const fnUpdater = (
    ...
    //     if (
    //       item.data !== undefined &&
    //       ScenarioOperationTypeActions.isScenarioOperationTypeEntityEqual(t, item.data)
    //     ) {
    //       return item.data;
    //     }
  ...
  const submit = (
    values: Partial<iot.ScenarioOperationTypeEntity>,
    formikProps?: FormikHelpers<Partial<iot.ScenarioOperationTypeEntity>>
