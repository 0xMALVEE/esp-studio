...
export function useGetNodeDatumsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getNodeDatumsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*iot.NodeDatumEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: iot.NodeDatumEntity[] = query$.data?.data?.items;
  const items = [];
