...
export function usePatchNodeWriter({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchNodeWriter(entity);

  const mutation = useMutation<
    IResponse<iot.NodeWriterEntity>,
    IResponse<iot.NodeWriterEntity>,
    Partial<iot.NodeWriterEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<iot.NodeWriterEntity>,
    formikProps?: FormikHelpers<Partial<iot.NodeWriterEntity>>
