...
export function useGetPhysicalSectionsExport({
  ...
  const fn = () => Q().getPhysicalSectionsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*iot.PhysicalSectionEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: iot.PhysicalSectionEntity[] = query$.data?.data?.items;
  const items = [];
