...
export function useGetModbusTaskByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getModbusTaskByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getModbusTaskByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
