...
export function usePatchModbusTransmissionMode({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchModbusTransmissionMode(entity);

  const mutation = useMutation<
    IResponse<iot.ModbusTransmissionModeEntity>,
    IResponse<iot.ModbusTransmissionModeEntity>,
    Partial<iot.ModbusTransmissionModeEntity>
  >(fn);
  ...
  const fnUpdater = (
    ...
    //     if (
    //       item.data !== undefined &&
    //       ModbusTransmissionModeActions.isModbusTransmissionModeEntityEqual(t, item.data)
    //     ) {
    //       return item.data;
    //     }
  ...
  const submit = (
    values: Partial<iot.ModbusTransmissionModeEntity>,
    formikProps?: FormikHelpers<Partial<iot.ModbusTransmissionModeEntity>>
