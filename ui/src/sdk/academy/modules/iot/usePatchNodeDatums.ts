...
export function usePatchNodeDatums({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchNodeDatums(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<iot.NodeDatumEntity>>,
    IResponse<core.BulkRecordRequest<iot.NodeDatumEntity>>,
    Partial<core.BulkRecordRequest<iot.NodeDatumEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<iot.NodeDatumEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<iot.NodeDatumEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<iot.NodeDatumEntity>>,
