...
export function useGetScenarioOperationTypesExport({
  ...
  const fn = () => Q().getScenarioOperationTypesExport();
  ...
  const query$ = useQuery(
    ["*iot.ScenarioOperationTypeEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: iot.ScenarioOperationTypeEntity[] = query$.data?.data?.items;
  const items = [];
