...
export function usePatchTriggerTypes({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchTriggerTypes(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<iot.TriggerTypeEntity>>,
    IResponse<core.BulkRecordRequest<iot.TriggerTypeEntity>>,
    Partial<core.BulkRecordRequest<iot.TriggerTypeEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<iot.TriggerTypeEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<iot.TriggerTypeEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<iot.TriggerTypeEntity>>,
