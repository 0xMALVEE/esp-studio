...
export function useGetDataNodeModeByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getDataNodeModeByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getDataNodeModeByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
