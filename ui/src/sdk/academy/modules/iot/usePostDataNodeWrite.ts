...
export function usePostDataNodeWrite({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().postDataNodeWrite(entity);

  const mutation = useMutation<
    IResponse<iot.NodeDatumEntity>,
    IResponse<iot.NodeDatumEntity>,
    Partial<iot.WriteDatumDto>
  >(fn);
  ...
  const fnUpdater = (
    ...
    //     if (
    //       item.data !== undefined &&
    //       DataNodeActions.isNodeDatumEntityEqual(t, item.data)
    //     ) {
    //       return item.data;
    //     }
  ...
  const submit = (
    values: Partial<iot.WriteDatumDto>,
