...
export function usePatchModbusTasks({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchModbusTasks(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<iot.ModbusTaskEntity>>,
    IResponse<core.BulkRecordRequest<iot.ModbusTaskEntity>>,
    Partial<core.BulkRecordRequest<iot.ModbusTaskEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<iot.ModbusTaskEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<iot.ModbusTaskEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<iot.ModbusTaskEntity>>,
