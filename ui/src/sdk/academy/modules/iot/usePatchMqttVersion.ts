...
export function usePatchMqttVersion({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchMqttVersion(entity);

  const mutation = useMutation<
    IResponse<iot.MqttVersionEntity>,
    IResponse<iot.MqttVersionEntity>,
    Partial<iot.MqttVersionEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<iot.MqttVersionEntity>,
    formikProps?: FormikHelpers<Partial<iot.MqttVersionEntity>>
