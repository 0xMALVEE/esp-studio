...
export function useGetDataNodeModesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getDataNodeModesExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*iot.DataNodeModeEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: iot.DataNodeModeEntity[] = query$.data?.data?.items;
  const items = [];
