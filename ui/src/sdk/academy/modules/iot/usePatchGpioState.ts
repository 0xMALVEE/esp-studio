...
export function usePatchGpioState({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchGpioState(entity);

  const mutation = useMutation<
    IResponse<iot.GpioStateEntity>,
    IResponse<iot.GpioStateEntity>,
    Partial<iot.GpioStateEntity>
  >(fn);
  ...
  const submit = (
    values: Partial<iot.GpioStateEntity>,
    formikProps?: FormikHelpers<Partial<iot.GpioStateEntity>>
