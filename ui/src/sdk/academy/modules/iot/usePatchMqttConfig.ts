...
export function usePatchMqttConfig({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchMqttConfig(entity);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<iot.MqttConfigDto>,
    response: IResponse<core.BulkRecordRequest<iot.MqttConfigDto>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
