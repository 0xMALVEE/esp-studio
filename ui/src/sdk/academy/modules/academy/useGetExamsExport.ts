...
export function useGetExamsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.ExamEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.ExamEntity[] = query$.data?.data?.items;
  const items = [];
