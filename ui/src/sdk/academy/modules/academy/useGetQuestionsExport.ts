...
export function useGetQuestionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.QuestionEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.QuestionEntity[] = query$.data?.data?.items;
  const items = [];
