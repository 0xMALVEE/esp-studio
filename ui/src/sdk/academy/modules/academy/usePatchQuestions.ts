...
export function usePatchQuestions({
  queryClient,
  query,
  execFnOverride,
}: {
  ...
  const fn = (entity: any) => Q().patchQuestions(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<academy.QuestionEntity>>,
    IResponse<core.BulkRecordRequest<academy.QuestionEntity>>,
    Partial<core.BulkRecordRequest<academy.QuestionEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<academy.QuestionEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<academy.QuestionEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<academy.QuestionEntity>>,
