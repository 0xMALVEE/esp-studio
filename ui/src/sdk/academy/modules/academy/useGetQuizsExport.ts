...
export function useGetQuizsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.QuizEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.QuizEntity[] = query$.data?.data?.items;
  const items = [];
