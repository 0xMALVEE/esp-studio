...
export function useGetAcTasksExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.AcTaskEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.AcTaskEntity[] = query$.data?.data?.items;
  const items = [];
