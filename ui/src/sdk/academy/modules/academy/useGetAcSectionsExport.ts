...
export function useGetAcSectionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getAcSectionsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.AcSectionEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.AcSectionEntity[] = query$.data?.data?.items;
  const items = [];
