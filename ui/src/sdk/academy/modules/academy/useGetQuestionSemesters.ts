...
export function useGetQuestionSemesters({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getQuestionSemesters();
  ...
  const query$ = useQuery(
    ["*academy.QuestionSemesterEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: academy.QuestionSemesterEntity[] = query$.data?.data?.items;
  const items = [];
