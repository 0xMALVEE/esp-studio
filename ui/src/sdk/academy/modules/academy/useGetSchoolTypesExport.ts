...
export function useGetSchoolTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getSchoolTypesExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.SchoolTypeEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.SchoolTypeEntity[] = query$.data?.data?.items;
  const items = [];
