...
export function usePatchQuizs({
  ...
  const fn = (entity: any) => Q().patchQuizs(entity);

  const mutation = useMutation<
    IResponse<core.BulkRecordRequest<academy.QuizEntity>>,
    IResponse<core.BulkRecordRequest<academy.QuizEntity>>,
    Partial<core.BulkRecordRequest<academy.QuizEntity>>
  >(fn);

  // Only entities are having a store in front-end

  const fnUpdater: any = (
    data: PossibleStoreData<core.BulkRecordRequest<academy.QuizEntity>>,
    response: IResponse<
      core.BulkRecordRequest<core.BulkRecordRequest<academy.QuizEntity>>
    >
  ) => {
    if (!data || !data.data) {
      return data;
  ...
  const submit = (
    values: Partial<core.BulkRecordRequest<academy.QuizEntity>>,
