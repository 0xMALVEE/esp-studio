...
export function useGetStudyYearsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getStudyYearsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.StudyYearEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.StudyYearEntity[] = query$.data?.data?.items;
  const items = [];
