...
export function useGetExamSessionReviewsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getExamSessionReviewsExport();
  ...
  const query$ = useQuery(
    ["*academy.ExamSessionReviewEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: academy.ExamSessionReviewEntity[] = query$.data?.data?.items;
  const items = [];
