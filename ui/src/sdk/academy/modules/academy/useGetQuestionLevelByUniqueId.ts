...
export function useGetQuestionLevelByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getQuestionLevelByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getQuestionLevelByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
