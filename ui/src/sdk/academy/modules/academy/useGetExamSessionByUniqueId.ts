...
export function useGetExamSessionByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getExamSessionByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getExamSessionByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
