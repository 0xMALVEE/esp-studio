...
export function useGetCourseEnrollmentsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getCourseEnrollmentsExport();
  ...
  const query$ = useQuery(
    ["*academy.CourseEnrollmentEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: academy.CourseEnrollmentEntity[] = query$.data?.data?.items;
  const items = [];
