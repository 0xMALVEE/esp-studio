...
export function useGetExamSessionProgress({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getExamSessionProgress();
  ...
  const query$ = useQuery(
    ["*academy.CommonCEFRSectionProgressDto", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: academy.CommonCEFRSectionProgressDto[] = query$.data?.data?.items;
  const items = [];
