...
export function useGetQuestionLevelsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getQuestionLevelsExport();
  ...
  const query$ = useQuery(
    ["*academy.QuestionLevelEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: academy.QuestionLevelEntity[] = query$.data?.data?.items;
  const items = [];
