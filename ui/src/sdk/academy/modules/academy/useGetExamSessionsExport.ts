...
export function useGetExamSessionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getExamSessionsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.ExamSessionEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.ExamSessionEntity[] = query$.data?.data?.items;
  const items = [];
