...
export function useGetQuestionDifficulityLevelsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getQuestionDifficulityLevelsExport();
  ...
  const query$ = useQuery(
    ["*academy.QuestionDifficulityLevelEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: academy.QuestionDifficulityLevelEntity[] = query$.data?.data?.items;
  const items = [];
