...
export function useGetUnitsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*academy.UnitEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: academy.UnitEntity[] = query$.data?.data?.items;
  const items = [];
