...
export function useGetQuestionBankByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getQuestionBankByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getQuestionBankByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
