...
export function useGetCapabilitysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getCapabilitysExport();
  ...
  const query$ = useQuery(
    ["*workspaces.CapabilityEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: workspaces.CapabilityEntity[] = query$.data?.data?.items;
  const items = [];
