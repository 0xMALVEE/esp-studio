...
export function useGetWorkspacesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getWorkspacesExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*workspaces.WorkspaceEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: workspaces.WorkspaceEntity[] = query$.data?.data?.items;
  const items = [];
