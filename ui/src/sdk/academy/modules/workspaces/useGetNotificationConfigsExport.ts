...
export function useGetNotificationConfigsExport({
  ...
  const fn = () => Q().getNotificationConfigsExport();
  ...
  const query$ = useQuery(
    ["*workspaces.NotificationConfigEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: workspaces.NotificationConfigEntity[] = query$.data?.data?.items;
  const items = [];
