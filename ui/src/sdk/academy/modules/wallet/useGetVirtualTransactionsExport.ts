...
export function useGetVirtualTransactionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fnx = execFnOverride
    ? VirtualTransactionActions.fnExec(execFnOverride(options))
    : execFn
  ...
  const fn = () => Q().getVirtualTransactionsExport();
  ...
  const query$ = useQuery(
    ["*wallet.VirtualTransactionEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: wallet.VirtualTransactionEntity[] = query$.data?.data?.items;
  const items = [];
