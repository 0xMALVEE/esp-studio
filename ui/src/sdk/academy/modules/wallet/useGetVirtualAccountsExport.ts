...
export function useGetVirtualAccountsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getVirtualAccountsExport();
  ...
  const query$ = useQuery(
    ["*wallet.VirtualAccountEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: wallet.VirtualAccountEntity[] = query$.data?.data?.items;
  const items = [];
