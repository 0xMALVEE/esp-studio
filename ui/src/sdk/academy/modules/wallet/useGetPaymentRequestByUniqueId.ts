...
export function useGetPaymentRequestByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getPaymentRequestByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getPaymentRequestByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
