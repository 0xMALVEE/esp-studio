...
export function useGetVirtualAccountByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getVirtualAccountByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getVirtualAccountByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
