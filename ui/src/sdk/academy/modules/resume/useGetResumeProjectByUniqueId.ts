...
export function useGetResumeProjectByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getResumeProjectByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getResumeProjectByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
