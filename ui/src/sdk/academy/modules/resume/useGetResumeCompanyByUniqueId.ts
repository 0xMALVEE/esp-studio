...
export function useGetResumeCompanyByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getResumeCompanyByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getResumeCompanyByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
