...
export function useGetResumeProjectTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getResumeProjectTypesExport();
  ...
  const query$ = useQuery(
    ["*resume.ResumeProjectTypeEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: resume.ResumeProjectTypeEntity[] = query$.data?.data?.items;
  const items = [];
