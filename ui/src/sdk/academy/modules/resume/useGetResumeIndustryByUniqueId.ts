...
export function useGetResumeIndustryByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getResumeIndustryByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getResumeIndustryByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
