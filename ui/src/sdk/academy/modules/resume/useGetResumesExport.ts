...
export function useGetResumesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*resume.ResumeEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: resume.ResumeEntity[] = query$.data?.data?.items;
  const items = [];
