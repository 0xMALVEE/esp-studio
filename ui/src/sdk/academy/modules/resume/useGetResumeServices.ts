...
export function useGetResumeServices({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*resume.ResumeServiceEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: resume.ResumeServiceEntity[] = query$.data?.data?.items;
  const items = [];
