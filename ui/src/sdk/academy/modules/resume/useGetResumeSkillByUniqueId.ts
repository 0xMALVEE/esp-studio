...
export function useGetResumeSkillByUniqueId({
  queryOptions,
  execFnOverride,
  query,
  queryClient,
  ...
  const fn = () => Q().getResumeSkillByUniqueId(query.uniqueId);
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery([options, query, "getResumeSkillByUniqueId"], fn, {
    cacheTime: 1001,
    retry: false,
