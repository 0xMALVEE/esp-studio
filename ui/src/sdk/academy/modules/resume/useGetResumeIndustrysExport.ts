...
export function useGetResumeIndustrysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getResumeIndustrysExport();
  ...
  const query$ = useQuery(
    ["*resume.ResumeIndustryEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: resume.ResumeIndustryEntity[] = query$.data?.data?.items;
  const items = [];
