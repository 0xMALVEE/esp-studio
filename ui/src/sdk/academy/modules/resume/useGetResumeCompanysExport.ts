...
export function useGetResumeCompanysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getResumeCompanysExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*resume.ResumeCompanyEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: resume.ResumeCompanyEntity[] = query$.data?.data?.items;
  const items = [];
