...
export function useGetResumeProductsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getResumeProductsExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*resume.ResumeProductEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: resume.ResumeProductEntity[] = query$.data?.data?.items;
  const items = [];
