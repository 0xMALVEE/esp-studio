...
export function useGetCommonProfilesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getCommonProfilesExport();
  ...
  const query$ = useQuery(
    ["*commonprofile.CommonProfileEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: commonprofile.CommonProfileEntity[] = query$.data?.data?.items;
  const items = [];
