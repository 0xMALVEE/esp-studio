...
export function useGetSkillsetCategorysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getSkillsetCategorysExport();
  ...
  const query$ = useQuery(
    ["*skillset.SkillsetCategoryEntity", options, query],
    fn,
    {
      cacheTime: 1000,
      retry: false,
  ...
  // const items: skillset.SkillsetCategoryEntity[] = query$.data?.data?.items;
  const items = [];
