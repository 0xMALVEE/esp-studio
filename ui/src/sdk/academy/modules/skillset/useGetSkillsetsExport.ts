...
export function useGetSkillsetsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*skillset.SkillsetEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: skillset.SkillsetEntity[] = query$.data?.data?.items;
  const items = [];
