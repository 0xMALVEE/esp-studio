...
export function useGetWidgetAreasExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const fn = () => Q().getWidgetAreasExport();
  ...
  const hasKey =
    ...
    auth != "null" &&
    !!auth;
  const query$ = useQuery(["*widget.WidgetAreaEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  // const items: widget.WidgetAreaEntity[] = query$.data?.data?.items;
  const items = [];
