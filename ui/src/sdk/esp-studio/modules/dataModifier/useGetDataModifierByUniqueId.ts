...
export function useGetDataModifierByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/dataModifier/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*dataModifier.DataModifierEntity"], fn, {
    cacheTime: 1001,
    retry: false,
