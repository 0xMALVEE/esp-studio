...
export function usePatchPipelines({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<PipelineEntity>,
    response: IResponse<core.BulkRecordRequest<PipelineEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PipelineEntity>) {
          queryClient.setQueriesData("*core.BulkRecordRequest[dataModifier.PipelineEntity]", (data) =>
