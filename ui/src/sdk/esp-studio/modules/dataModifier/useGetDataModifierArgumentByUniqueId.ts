...
export function useGetDataModifierArgumentByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/dataModifierArgument/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*dataModifier.DataModifierArgumentEntity"], fn, {
    cacheTime: 1001,
    retry: false,
