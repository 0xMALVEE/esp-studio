...
export function useDeletePipeline({execFnOverride, queryClient, query}: {queryClient: QueryClient, execFnOverride?: any, query?: any}) {
      ...
      mutation.mutate(values, {
        onSuccess(response: IDeleteResponse) {
          queryClient.setQueryData<IDeleteResponse>(
            "*dataModifier.PipelineEntity",
            (data) => fnUpdater(data, response)
          );
          queryClient.invalidateQueries("*dataModifier.PipelineEntity");
          resolve(response);
        },
        onError(error: any) {
