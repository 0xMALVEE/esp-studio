...
export function useGetPipelinesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/pipelines/export".substr(1);
  ...
  const query$ = useQuery(["*dataModifier.PipelineEntity", options, query], fn, {
    cacheTime: 1000,
    retry: false,
...
useGetPipelinesExport.UKEY = "*dataModifier.PipelineEntity"