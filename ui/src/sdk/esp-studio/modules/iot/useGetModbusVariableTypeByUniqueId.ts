...
export function useGetModbusVariableTypeByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/modbus-variable-type/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.ModbusVariableTypeEntity"], fn, {
    cacheTime: 1001,
    retry: false,
