...
export function useGetDataNodesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/data-nodes/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DataNodeEntity>, any>(["*iot.DataNodeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DataNodeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDataNodesExport.UKEY = "*iot.DataNodeEntity"