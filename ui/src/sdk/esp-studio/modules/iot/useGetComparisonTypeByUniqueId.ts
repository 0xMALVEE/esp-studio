...
export function useGetComparisonTypeByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/comparisonType/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*iot.ComparisonTypeEntity"], fn, {
    cacheTime: 1001,
    retry: false,
