...
export function usePostTriggerType({queryClient, query, execFnOverride}: UseRemoteQuery) {
  query = query || {}
  ...
  const url = "/triggerType".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<TriggerTypeEntity> | undefined,
    item: IResponse<TriggerTypeEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<TriggerTypeEntity>) {
          queryClient.setQueryData<IResponseList<TriggerTypeEntity>>(
