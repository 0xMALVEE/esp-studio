...
export function useGetMovableObjectsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/movable-objects/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<MovableObjectEntity>, any>(["*iot.MovableObjectEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<MovableObjectEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetMovableObjectsExport.UKEY = "*iot.MovableObjectEntity"