...
export function useGetGpioStatesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/gpio-states/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<GpioStateEntity>, any>(["*iot.GpioStateEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<GpioStateEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetGpioStatesExport.UKEY = "*iot.GpioStateEntity"