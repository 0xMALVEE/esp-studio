...
export function usePostDevice(props?: UseRemoteQuery) {
  ...
  const fnUpdater = (
    data: IResponseList<DeviceEntity> | undefined,
    item: IResponse<DeviceEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<DeviceEntity>) {
          queryClient?.setQueryData<IResponseList<DeviceEntity>>(
            "*iot.DeviceEntity",
