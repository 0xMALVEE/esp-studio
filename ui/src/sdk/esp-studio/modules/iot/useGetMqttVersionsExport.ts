...
export function useGetMqttVersionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/mqtt-versions/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<MqttVersionEntity>, any>(["*iot.MqttVersionEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<MqttVersionEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetMqttVersionsExport.UKEY = "*iot.MqttVersionEntity"