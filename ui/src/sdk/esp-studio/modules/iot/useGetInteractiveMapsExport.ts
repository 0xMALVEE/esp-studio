...
export function useGetInteractiveMapsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/interactive-maps/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<InteractiveMapEntity>, any>(["*iot.InteractiveMapEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<InteractiveMapEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetInteractiveMapsExport.UKEY = "*iot.InteractiveMapEntity"