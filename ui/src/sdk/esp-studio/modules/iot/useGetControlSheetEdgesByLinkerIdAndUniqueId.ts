...
export function useGetControlSheetEdgesByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/control-sheet/edges/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.ControlSheetEdges"], fn, {
    cacheTime: 1001,
    retry: false,
