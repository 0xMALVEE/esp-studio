...
export function useGetFlowValuesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/flow-values/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<FlowValueEntity>, any>(["*iot.FlowValueEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<FlowValueEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetFlowValuesExport.UKEY = "*iot.FlowValueEntity"