...
export function usePatchMovableObjects({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/movable-objects".substr(1);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<MovableObjectEntity>,
    response: IResponse<BulkRecordRequest<MovableObjectEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<MovableObjectEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[iot.MovableObjectEntity]", (data: any) =>
