...
export function useGetHmisExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/hmis/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<HmiEntity>, any>(["*iot.HmiEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<HmiEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetHmisExport.UKEY = "*iot.HmiEntity"