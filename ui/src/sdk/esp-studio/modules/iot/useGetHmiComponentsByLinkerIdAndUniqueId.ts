...
export function useGetHmiComponentsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/hmi/components/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.HmiComponents"], fn, {
    cacheTime: 1001,
    retry: false,
