...
export function useGetGpioModesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/gpio-modes/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<GpioModeEntity>, any>(["*iot.GpioModeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<GpioModeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetGpioModesExport.UKEY = "*iot.GpioModeEntity"