...
export function useGetDataNodeWritersByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/data-node/writers/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.DataNodeWriters"], fn, {
    cacheTime: 1001,
    retry: false,
