...
export function useGetHmiComponentTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/hmi-component-types/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<HmiComponentTypeEntity>, any>(["*iot.HmiComponentTypeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<HmiComponentTypeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetHmiComponentTypesExport.UKEY = "*iot.HmiComponentTypeEntity"