...
export function useGetDeviceDeviceModbusConfigByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/device/device_modbus_config/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.DeviceDeviceModbusConfig"], fn, {
    cacheTime: 1001,
    retry: false,
