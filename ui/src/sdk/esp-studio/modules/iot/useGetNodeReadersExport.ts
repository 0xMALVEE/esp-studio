...
export function useGetNodeReadersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/node-readers/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<NodeReaderEntity>, any>(["*iot.NodeReaderEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<NodeReaderEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetNodeReadersExport.UKEY = "*iot.NodeReaderEntity"