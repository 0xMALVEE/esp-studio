...
export function usePostGpio(props?: UseRemoteQuery) {
  ...
  const fnUpdater = (
    data: IResponseList<GpioEntity> | undefined,
    item: IResponse<GpioEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<GpioEntity>) {
          queryClient?.setQueryData<IResponseList<GpioEntity>>(
            "*iot.GpioEntity",
