...
export function usePatchModbusFunctionCodes({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/modbus-function-codes".substr(1);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<ModbusFunctionCodeEntity>,
    response: IResponse<BulkRecordRequest<ModbusFunctionCodeEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<ModbusFunctionCodeEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[iot.ModbusFunctionCodeEntity]", (data: any) =>
