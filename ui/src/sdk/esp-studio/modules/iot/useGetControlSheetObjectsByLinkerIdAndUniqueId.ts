...
export function useGetControlSheetObjectsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/control-sheet/objects/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.ControlSheetObjects"], fn, {
    cacheTime: 1001,
    retry: false,
