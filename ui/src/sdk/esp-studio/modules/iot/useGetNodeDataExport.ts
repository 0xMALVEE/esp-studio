...
export function useGetNodeDataExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/node-data/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<NodeDatumEntity>, any>(["*iot.NodeDatumEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<NodeDatumEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetNodeDataExport.UKEY = "*iot.NodeDatumEntity"