...
export function useGetScenarioScriptByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/scenario/script/:linkerId/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*iot.ScenarioScriptEntity"], fn, {
    cacheTime: 1001,
    retry: false,
