...
export function usePatchComparisonTypes({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/comparisonTypes".substr(1);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<ComparisonTypeEntity>,
    response: IResponse<core.BulkRecordRequest<ComparisonTypeEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<ComparisonTypeEntity>) {
          queryClient.setQueriesData("*core.BulkRecordRequest[iot.ComparisonTypeEntity]", (data) =>
