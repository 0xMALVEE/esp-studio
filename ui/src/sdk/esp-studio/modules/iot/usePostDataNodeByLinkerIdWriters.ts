...
export function usePostDataNodeByLinkerIdWriters(props?: UseRemoteQuery) {
  ...
  const url = "/data-node/:linkerId/writers".substr(1);
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<DataNodeWriters>) {
          queryClient?.setQueryData<IResponseList<DataNodeWriters>>(
            "*iot.DataNodeWriters",
