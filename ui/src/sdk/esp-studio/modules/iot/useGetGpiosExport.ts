...
export function useGetGpiosExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/gpios/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<GpioEntity>, any>(["*iot.GpioEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<GpioEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetGpiosExport.UKEY = "*iot.GpioEntity"