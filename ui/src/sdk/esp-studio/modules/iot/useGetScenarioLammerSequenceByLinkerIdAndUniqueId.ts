...
export function useGetScenarioLammerSequenceByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/scenario/lammerSequence/:linkerId/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*iot.ScenarioLammerSequenceEntity"], fn, {
    cacheTime: 1001,
    retry: false,
