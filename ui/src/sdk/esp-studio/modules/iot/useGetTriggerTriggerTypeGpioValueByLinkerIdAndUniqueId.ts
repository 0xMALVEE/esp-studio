...
export function useGetTriggerTriggerTypeGpioValueByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
    unauthorized 
}: { query?: any , queryClient?: QueryClient, unauthorized?: boolean, execFnOverride?: any, queryOptions?: UseQueryOptions<any>}) {
  ...
  const url = "/trigger/triggerTypeGpioValue/:linkerId/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*iot.TriggerTriggerTypeGpioValueEntity"], fn, {
    cacheTime: 1001,
    retry: false,
