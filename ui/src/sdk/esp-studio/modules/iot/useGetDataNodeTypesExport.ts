...
export function useGetDataNodeTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/data-node-types/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DataNodeTypeEntity>, any>(["*iot.DataNodeTypeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DataNodeTypeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDataNodeTypesExport.UKEY = "*iot.DataNodeTypeEntity"