...
export function useGetModbusFunctionCodeByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/modbus-function-code/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.ModbusFunctionCodeEntity"], fn, {
    cacheTime: 1001,
    retry: false,
