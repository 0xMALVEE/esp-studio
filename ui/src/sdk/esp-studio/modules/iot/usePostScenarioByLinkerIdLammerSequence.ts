...
export function usePostScenarioByLinkerIdLammerSequence({queryClient, query, execFnOverride}: UseRemoteQuery) {
  query = query || {}
  ...
  const url = "/scenario/:linkerId/lammerSequence".substr(1);
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<ScenarioLammerSequenceEntity>) {
          queryClient.setQueryData<IResponseList<ScenarioLammerSequenceEntity>>(
            "*iot.ScenarioLammerSequenceEntity",
