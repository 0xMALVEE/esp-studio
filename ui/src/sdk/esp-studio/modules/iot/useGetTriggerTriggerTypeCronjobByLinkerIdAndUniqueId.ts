...
export function useGetTriggerTriggerTypeCronjobByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/trigger/triggerTypeCronjob/:linkerId/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*iot.TriggerTriggerTypeCronjobEntity"], fn, {
    cacheTime: 1001,
    retry: false,
