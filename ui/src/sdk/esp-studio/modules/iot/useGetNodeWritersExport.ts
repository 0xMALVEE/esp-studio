...
export function useGetNodeWritersExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/node-writers/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<NodeWriterEntity>, any>(["*iot.NodeWriterEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<NodeWriterEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetNodeWritersExport.UKEY = "*iot.NodeWriterEntity"