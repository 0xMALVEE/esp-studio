...
export function useGetDataNodeReadersByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/data-node/readers/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.DataNodeReaders"], fn, {
    cacheTime: 1001,
    retry: false,
