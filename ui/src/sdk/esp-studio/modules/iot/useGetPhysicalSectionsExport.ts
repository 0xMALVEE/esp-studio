...
export function useGetPhysicalSectionsExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/physical-sections/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<PhysicalSectionEntity>, any>(["*iot.PhysicalSectionEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<PhysicalSectionEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetPhysicalSectionsExport.UKEY = "*iot.PhysicalSectionEntity"