...
export function useGetDeviceTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/device-types/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DeviceTypeEntity>, any>(["*iot.DeviceTypeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DeviceTypeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDeviceTypesExport.UKEY = "*iot.DeviceTypeEntity"