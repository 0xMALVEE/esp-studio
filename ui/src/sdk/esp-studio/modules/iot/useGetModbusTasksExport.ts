...
export function useGetModbusTasksExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/modbus-tasks/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<ModbusTaskEntity>, any>(["*iot.ModbusTaskEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<ModbusTaskEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetModbusTasksExport.UKEY = "*iot.ModbusTaskEntity"