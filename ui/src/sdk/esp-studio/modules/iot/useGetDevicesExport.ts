...
export function useGetDevicesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/devices/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<DeviceEntity>, any>(["*iot.DeviceEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<DeviceEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetDevicesExport.UKEY = "*iot.DeviceEntity"