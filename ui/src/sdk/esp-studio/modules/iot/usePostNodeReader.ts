...
export function usePostNodeReader(props?: UseRemoteQuery) {
  ...
  const url = "/node-reader".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<NodeReaderEntity> | undefined,
    item: IResponse<NodeReaderEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<NodeReaderEntity>) {
          queryClient?.setQueryData<IResponseList<NodeReaderEntity>>(
