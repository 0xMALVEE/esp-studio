...
export function useGetPhysicalSectionByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/physical-section/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.PhysicalSectionEntity"], fn, {
    cacheTime: 1001,
    retry: false,
