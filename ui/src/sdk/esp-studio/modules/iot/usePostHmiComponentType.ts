...
export function usePostHmiComponentType(props?: UseRemoteQuery) {
  ...
  const url = "/hmi-component-type".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<HmiComponentTypeEntity> | undefined,
    item: IResponse<HmiComponentTypeEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<HmiComponentTypeEntity>) {
          queryClient?.setQueryData<IResponseList<HmiComponentTypeEntity>>(
            "*iot.HmiComponentTypeEntity",
