...
export function useGetModbusVariableTypesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/modbus-variable-types/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<ModbusVariableTypeEntity>, any>(["*iot.ModbusVariableTypeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<ModbusVariableTypeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetModbusVariableTypesExport.UKEY = "*iot.ModbusVariableTypeEntity"