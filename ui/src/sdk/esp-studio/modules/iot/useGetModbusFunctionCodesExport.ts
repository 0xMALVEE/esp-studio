...
export function useGetModbusFunctionCodesExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/modbus-function-codes/export".substr(1);
  ...
  const query$ = useQuery<any, any, IResponseList<ModbusFunctionCodeEntity>, any>(["*iot.ModbusFunctionCodeEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
  ...
  const items: Array<ModbusFunctionCodeEntity> = query$.data?.data?.items || [];
  return { query: query$, items};
}
useGetModbusFunctionCodesExport.UKEY = "*iot.ModbusFunctionCodeEntity"