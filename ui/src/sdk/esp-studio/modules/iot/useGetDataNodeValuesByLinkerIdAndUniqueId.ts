...
export function useGetDataNodeValuesByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/data-node/values/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*iot.DataNodeValues"], fn, {
    cacheTime: 1001,
    retry: false,
