...
export function usePostTrigger({queryClient, query, execFnOverride}: UseRemoteQuery) {
  query = query || {}
  ...
  const fnUpdater = (
    data: IResponseList<TriggerEntity> | undefined,
    item: IResponse<TriggerEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<TriggerEntity>) {
          queryClient.setQueryData<IResponseList<TriggerEntity>>(
            "*iot.TriggerEntity",
