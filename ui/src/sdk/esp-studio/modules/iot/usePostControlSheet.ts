...
export function usePostControlSheet(props?: UseRemoteQuery) {
  ...
  const url = "/control-sheet".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<ControlSheetEntity> | undefined,
    item: IResponse<ControlSheetEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<ControlSheetEntity>) {
          queryClient?.setQueryData<IResponseList<ControlSheetEntity>>(
            "*iot.ControlSheetEntity",
