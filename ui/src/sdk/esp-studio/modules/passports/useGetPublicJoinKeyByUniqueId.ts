...
export function useGetPublicJoinKeyByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/public-join-key/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*workspaces.PublicJoinKeyEntity"], fn, {
    cacheTime: 1001,
    retry: false,
