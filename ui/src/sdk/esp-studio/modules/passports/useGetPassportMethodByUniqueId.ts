...
export function useGetPassportMethodByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/passport-method/:uniqueId".substr(1);
...
computedUrl = computedUrl.replace(":uniqueId", query[":uniqueId".replace(":", "")])  
  ...
  const query$ = useQuery([options, query, "*workspaces.PassportMethodEntity"], fn, {
    cacheTime: 1001,
    retry: false,
