...
export function useGetPriceTagVariationsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/price-tag/variations/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*currency.PriceTagVariations"], fn, {
    cacheTime: 1001,
    retry: false,
