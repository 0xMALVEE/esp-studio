...
export function usePatchPriceTags({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  ...
  const fnUpdater: any = (
    ...
    response: IResponse<core.BulkRecordRequest<PriceTagEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PriceTagEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[currency.PriceTagEntity]", (data) =>
