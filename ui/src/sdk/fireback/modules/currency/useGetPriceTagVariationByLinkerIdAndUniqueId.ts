...
export function useGetPriceTagVariationByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/priceTag/variation/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*currency.PriceTagVariationEntity"], fn, {
    cacheTime: 1001,
    retry: false,
