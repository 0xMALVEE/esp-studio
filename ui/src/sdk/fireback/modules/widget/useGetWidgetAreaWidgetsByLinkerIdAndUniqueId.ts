...
export function useGetWidgetAreaWidgetsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/widget-area/widgets/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*widget.WidgetAreaWidgets"], fn, {
    cacheTime: 1001,
    retry: false,
