...
export function usePostWidgetArea({queryClient, query, execFnOverride}: UseRemoteQuery) {
  query = query || {}
  ...
  const url = "/widget-area".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<WidgetAreaEntity> | undefined,
    item: IResponse<WidgetAreaEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<WidgetAreaEntity>) {
          queryClient.setQueryData<IResponseList<WidgetAreaEntity>>(
