...
export function usePatchLicensableProducts({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  ...
  const fnUpdater: any = (
    ...
    response: IResponse<core.BulkRecordRequest<LicensableProductEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<LicensableProductEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[licenses.LicensableProductEntity]", (data) =>
