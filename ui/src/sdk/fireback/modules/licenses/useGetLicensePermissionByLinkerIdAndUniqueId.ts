...
export function useGetLicensePermissionByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/license/permission/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*licenses.LicensePermissionEntity"], fn, {
    cacheTime: 1001,
    retry: false,
