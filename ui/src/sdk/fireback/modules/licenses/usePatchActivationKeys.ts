...
export function usePatchActivationKeys({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/activation-keys".substr(1);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<ActivationKeyEntity>,
    response: IResponse<core.BulkRecordRequest<ActivationKeyEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<ActivationKeyEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[licenses.ActivationKeyEntity]", (data) =>
