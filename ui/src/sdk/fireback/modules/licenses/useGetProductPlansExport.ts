...
export function useGetProductPlansExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/product-plans/export".substr(1);
  ...
  const query$ = useQuery(["*licenses.ProductPlanEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
