...
export function useGetActivationKeyByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/activation-key/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*licenses.ActivationKeyEntity"], fn, {
    cacheTime: 1001,
    retry: false,
