...
export function useGetProductPlanPermissionByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/productPlan/permission/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*licenses.ProductPlanPermissionEntity"], fn, {
    cacheTime: 1001,
    retry: false,
