...
export function usePostLicensableProductsGenerate({queryClient, query, execFnOverride}: UseRemoteQuery) {
  query = query || {}
  ...
  const url = "/licensableProducts/generate".substr(1);
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<LicensableProductEntity>) {
          queryClient.setQueryData<IResponseList<LicensableProductEntity>>(
