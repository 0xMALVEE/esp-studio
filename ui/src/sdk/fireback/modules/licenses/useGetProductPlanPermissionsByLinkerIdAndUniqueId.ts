...
export function useGetProductPlanPermissionsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/product-plan/permissions/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*licenses.ProductPlanPermissions"], fn, {
    cacheTime: 1001,
    retry: false,
