...
export function useGetLicensePermissionsByLinkerIdAndUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/license/permissions/:linkerId/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*licenses.LicensePermissions"], fn, {
    cacheTime: 1001,
    retry: false,
