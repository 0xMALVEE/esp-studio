...
export function usePatchLicensableProduct({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/licensable-product".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<LicensableProductEntity> | undefined,
    item: IResponse<LicensableProductEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<LicensableProductEntity>) {
          queryClient.setQueriesData("*licenses.LicensableProductEntity", (data) =>
