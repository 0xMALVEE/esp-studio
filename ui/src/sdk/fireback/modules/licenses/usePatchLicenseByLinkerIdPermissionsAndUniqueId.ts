...
import {
    LicensePermissions,
} from "../licenses/LicensePermissions"
export function usePatchLicenseByLinkerIdPermissionsAndUniqueId({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/license/:linkerId/permissions/:uniqueId".substr(1);
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<LicensePermissions>) {
          queryClient.setQueriesData("*licenses.LicensePermissions", (data) =>
