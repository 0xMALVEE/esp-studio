...
export function useGetLicensableProductByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/licensable-product/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*licenses.LicensableProductEntity"], fn, {
    cacheTime: 1001,
    retry: false,
