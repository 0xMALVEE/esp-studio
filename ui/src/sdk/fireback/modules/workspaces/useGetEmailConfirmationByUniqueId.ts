...
export function useGetEmailConfirmationByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/email-confirmation/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*workspaces.EmailConfirmationEntity"], fn, {
    cacheTime: 1001,
    retry: false,
