...
export function useGetUserWorkspaceByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/user-workspace/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*workspaces.UserWorkspaceEntity"], fn, {
    cacheTime: 1001,
    retry: false,
