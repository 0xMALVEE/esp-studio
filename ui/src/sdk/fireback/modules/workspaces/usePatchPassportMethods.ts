...
export function usePatchPassportMethods({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<PassportMethodEntity>,
    response: IResponse<core.BulkRecordRequest<PassportMethodEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PassportMethodEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.PassportMethodEntity]", (data) =>
