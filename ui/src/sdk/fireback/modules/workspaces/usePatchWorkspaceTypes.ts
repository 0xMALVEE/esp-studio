...
export function usePatchWorkspaceTypes({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  ...
  const fnUpdater: any = (
    ...
    response: IResponse<core.BulkRecordRequest<WorkspaceTypeEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<WorkspaceTypeEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.WorkspaceTypeEntity]", (data) =>
