...
export function usePostPassport({queryClient, query, execFnOverride}: UseRemoteQuery) {
  query = query || {}
  ...
  const fnUpdater = (
    data: IResponseList<PassportEntity> | undefined,
    item: IResponse<PassportEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PassportEntity>) {
          queryClient.setQueryData<IResponseList<PassportEntity>>(
