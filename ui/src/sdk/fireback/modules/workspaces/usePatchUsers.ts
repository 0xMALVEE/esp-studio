...
export function usePatchUsers({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<UserEntity>,
    response: IResponse<core.BulkRecordRequest<UserEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<UserEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.UserEntity]", (data) =>
