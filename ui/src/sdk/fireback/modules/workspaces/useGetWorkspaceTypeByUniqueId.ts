...
export function useGetWorkspaceTypeByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/workspace-type/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*workspaces.WorkspaceTypeEntity"], fn, {
    cacheTime: 1001,
    retry: false,
