...
export function usePatchWorkspaceRoles({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<WorkspaceRoleEntity>,
    response: IResponse<core.BulkRecordRequest<WorkspaceRoleEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<WorkspaceRoleEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.WorkspaceRoleEntity]", (data) =>
