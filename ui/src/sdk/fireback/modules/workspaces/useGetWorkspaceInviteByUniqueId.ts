...
export function useGetWorkspaceInviteByUniqueId({ 
    queryOptions,
    execFnOverride,
    query,
    queryClient,
  ...
  const url = "/workspace-invite/:uniqueId".substr(1);
  ...
  const query$ = useQuery([options, query, "*workspaces.WorkspaceInviteEntity"], fn, {
    cacheTime: 1001,
    retry: false,
