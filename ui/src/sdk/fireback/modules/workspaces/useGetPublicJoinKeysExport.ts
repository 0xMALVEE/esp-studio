...
export function useGetPublicJoinKeysExport({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/public-join-keys/export".substr(1);
  ...
  const query$ = useQuery(["*workspaces.PublicJoinKeyEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
