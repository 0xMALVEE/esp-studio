...
export function usePatchPublicJoinKeys({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<PublicJoinKeyEntity>,
    response: IResponse<core.BulkRecordRequest<PublicJoinKeyEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<PublicJoinKeyEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.PublicJoinKeyEntity]", (data) =>
