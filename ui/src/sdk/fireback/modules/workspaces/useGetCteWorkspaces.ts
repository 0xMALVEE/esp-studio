...
export function useGetCteWorkspaces({
  queryOptions,
  query,
  queryClient,
  execFnOverride,
  ...
  const url = "/cte-workspaces".substr(1);
  ...
  const query$ = useQuery(["*workspaces.WorkspaceEntity", computedOptions, query], fn, {
    cacheTime: 1000,
    retry: false,
...
useGetCteWorkspaces.UKEY = "*workspaces.WorkspaceEntity"