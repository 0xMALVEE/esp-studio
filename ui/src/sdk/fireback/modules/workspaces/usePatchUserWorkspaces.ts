...
export function usePatchUserWorkspaces({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/user-workspaces".substr(1);
  ...
  const fnUpdater: any = (
    data: PossibleStoreData<UserWorkspaceEntity>,
    response: IResponse<core.BulkRecordRequest<UserWorkspaceEntity>>
  ) => {
    if (!data || !data.data) {
      return data;
    }
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<UserWorkspaceEntity>) {
          queryClient.setQueriesData("*workspaces.BulkRecordRequest[workspaces.UserWorkspaceEntity]", (data) =>
