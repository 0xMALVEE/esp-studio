...
export function usePatchWorkspaceTypeDistinct({queryClient, query, execFnOverride}: {queryClient: QueryClient, query?: any, execFnOverride?: any}) {
  query = query || {}
  ...
  const url = "/workspace-type/distinct".substr(1);
  ...
  const fnUpdater = (
    data: IResponseList<WorkspaceTypeEntity> | undefined,
    item: IResponse<WorkspaceTypeEntity>
  ) => {
    if (!data) {
      return {
        data: { items: [] },
    ...
    return new Promise((resolve, reject) => {
      mutation.mutate(values, {
        onSuccess(response: IResponse<WorkspaceTypeEntity>) {
          queryClient.setQueriesData("*workspaces.WorkspaceTypeEntity", (data) =>
