...
const SortingIcon = ({ direction }: any) => (
  <>
    {direction === "asc" ? (
      <img className="table-sort-img asc" src={source(osResources.up)} />
    ) : direction === "desc" ? (
      <img className="table-sort-img desc" src={source(osResources.down)} />
    ) : null}
  </>
);

const SortLabel = ({ onSort, children, direction }: any) => (
  <span onClick={onSort} className="table-sort-btn">
    {children}
