#ifndef memory_stat_json_c
#define memory_stat_json_c
#include "cJSON.h"
...
cJSON *memory_stat_json_object(struct memory_stat_entity_t *data)
{
    ...
    if (data->heapSize != NULL) {
        cJSON_AddNumberToObject(json, "heapSize", *data->heapSize);
    }
    return json;
};
char *memory_stat_json(struct memory_stat_entity_t *data)
{
    cJSON *json = memory_stat_json_object(data);
...
{
    struct memory_stat_entity_t dto = {};
    ...
    if (cJSON_GetObjectItem(json, "heapSize") != NULL) {
            dto.heapSize = &cJSON_GetObjectItem(json, "heapSize")->valuedouble;
    }
    return dto;
};
#endif