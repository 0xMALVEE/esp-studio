...
static int device_write_ssid(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint8_t incoming[32];
    // uint8_t *incoming = (uint8_t)malloc(ctxt->om->om_len);
    memcpy(incoming, ctxt->om->om_data, ctxt->om->om_len);

    set_wifi_info("ssid", (char *)incoming);
    // free(incoming);
...
static int device_write_password(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint8_t incoming[32];
    // uint8_t *incoming = (uint8_t *)malloc(2);
    memcpy(incoming, ctxt->om->om_data, ctxt->om->om_len);

    set_wifi_info("password", (char *)incoming);
    // free(incoming);
    printf("PASSWORD: %.*s\n", ctxt->om->om_len, ctxt->om->om_data);
    return 0;
}
static int device_reset(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
...
static const struct ble_gatt_svc_def gatt_svcs[] = {
    {.type = BLE_GATT_SVC_TYPE_PRIMARY,
     ...
     .characteristics = (struct ble_gatt_chr_def[]){
         ...
         {.uuid = BLE_UUID16_DECLARE(0xDEBB), // Define UUID for writing
          .flags = BLE_GATT_CHR_F_WRITE,
          .access_cb = device_write_password},
         {.uuid = BLE_UUID16_DECLARE(0xDECC), // Define UUID for writing
          .flags = BLE_GATT_CHR_F_WRITE,
          .access_cb = device_reset},
         {0}}},
    {0}};
