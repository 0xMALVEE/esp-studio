#ifndef gpio_caction_c
#define gpio_caction_c
...
char* gpio_entity_to_sql_insert(struct gpio_entity_t *data)
{
    char* query = sqlite3_mprintf(
        gpio_insert_sql
            ...
            , data->analogFunction == NULL ? "" :  data->analogFunction
            , data->rtcGpio == NULL ? "" :  data->rtcGpio
...
char* gpio_entity_to_sql_update(struct gpio_entity_t *data)
{
    ...
    str_builder_add_str(sb, "UPDATE fb_gpio_entities set unique_id = unique_id ", 0);
        ...
        if (data->name != NULL) {
            str_builder_add_str(sb, ",name = '", 0);
            str_builder_add_str(sb, data->name, 0);
            str_builder_add_str(sb, "'", 0);
        }
        if (data->index != NULL) {
        ...
        if (data->analogFunction != NULL) {
            str_builder_add_str(sb, ",analog_function = '", 0);
            str_builder_add_str(sb, data->analogFunction, 0);
            str_builder_add_str(sb, "'", 0);
        }
        if (data->rtcGpio != NULL) {
            str_builder_add_str(sb, ",rtc_gpio = '", 0);
            str_builder_add_str(sb, data->rtcGpio, 0);
            str_builder_add_str(sb, "'", 0);
        }
        if (data->comments != NULL) {
            str_builder_add_str(sb, ",comments = '", 0);
            str_builder_add_str(sb, data->comments, 0);
            str_builder_add_str(sb, "'", 0);
        }
...
{
    int response;
    execute_sql(gpio_create_sql , dbref);
    return 0;
}
;
    ...
    while (sqlite3_step(stmt) != SQLITE_DONE) {
		...
		for (i = 0; i < num_cols; i++)
		{
            ...
            if (strcmp(name, "index") == 0) {
                ...
                // TEXT
            if (strcmp(name, "analog_function") == 0) {
                    cJSON_AddStringToObject(json, "analogFunction", (const char *)sqlite3_column_text(stmt, i));
            }
                // TEXT
            if (strcmp(name, "rtc_gpio") == 0) {
                    cJSON_AddStringToObject(json, "rtcGpio", (const char *)sqlite3_column_text(stmt, i));
            }
                // TEXT
            if (strcmp(name, "comments") == 0) {
                    cJSON_AddStringToObject(json, "comments", (const char *)sqlite3_column_text(stmt, i));
            }
        }
        record++;
        break;
    ...
    while (sqlite3_step(stmt) != SQLITE_DONE) {
		...
		for (i = 0; i < num_cols; i++)
		{
            ...
            if (strcmp(name, "index") == 0) {
                ...
                // TEXT
            if (strcmp(name, "analog_function") == 0) {
                    cJSON_AddStringToObject(row, "analogFunction", (const char *)sqlite3_column_text(stmt, i));
            }
                // TEXT
            if (strcmp(name, "rtc_gpio") == 0) {
                    cJSON_AddStringToObject(row, "rtcGpio", (const char *)sqlite3_column_text(stmt, i));
            }
                // TEXT
            if (strcmp(name, "comments") == 0) {
                    cJSON_AddStringToObject(row, "comments", (const char *)sqlite3_column_text(stmt, i));
            }
        }
        record++;
